...
```
As can be seen, the reference plane is translated to the work space defined by the position of a piece of paper and the location of the robotic arm. The non uniformity of the work space generates some particular cases that have to be improbed. In the python code _demo_gcode.py_ is presented some of the special cases that has been taken into account for our exercise.
